          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-[#CACEB5]/80">
            ...
            <div className="flex items-center gap-3">
              {result.pdf_report_path && (
                <a
                  href={result.pdf_report_path}
                  download
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 bg-[#1F2532] hover:bg-[#2F3543] text-white text-sm font-extrabold px-4 py-2.5 rounded-xl shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-pointer border border-[#CACEB5]/40"
                >
                ...
                <div className="border border-[#CACEB5] rounded-2xl p-5 bg-gradient-to-b from-white via-[#faf8f4] to-[#f4efe6]/50 space-y-4 hover:border-[#4E6158] shadow-2xs transition-all duration-200">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="text-sm font-black text-[#1F2532]">MRZ Checksum Validation</h4>
                        <p className="text-xs text-[#2F3543] mt-0.5 leading-relaxed font-semibold">
                          {result.checks.find(c => c.id === "mrz")?.detail || t("mrz_desc")}
                        </p>
                      </div>
                    </div>

                    <span className={`px-3 py-1 rounded-md text-xs font-extrabold border shrink-0 shadow-2xs ${
                      result.checks.find(c => c.id === "mrz")?.passed
                        ? "bg-gradient-to-r from-[#ECE6DA] via-[#f2ece0] to-[#e4ded0] text-[#4E6158] border-[#CACEB5]"
                        : "bg-red-50 text-red-700 border-red-200"
                    }`}>
                      {result.checks.find(c => c.id === "mrz")?.passed ? "Passed" : "Failed"}
                    </span>
                  </div>

                  {result.checks.find(c => c.id === "mrz")?.mrzData && (
                    <div className="bg-gradient-to-br from-[#f7f4ee] to-[#f0ebdf] border border-[#CACEB5]/80 rounded-xl p-3.5 font-mono text-[11px] sm:text-xs text-[#1F2532] font-black leading-relaxed overflow-x-auto shadow-inner">
                      {result.checks.find(c => c.id === "mrz")?.mrzData?.map((line, idx) => (
                ...
                <div className="border border-[#CACEB5] rounded-2xl p-5 bg-gradient-to-b from-white via-[#faf8f4] to-[#f4efe6]/50 space-y-4 hover:border-[#4E6158] shadow-2xs transition-all duration-200">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="text-sm font-black text-[#1F2532]">Field Cross-Verification</h4>
                        <p className="text-xs text-[#2F3543] mt-0.5 leading-relaxed font-semibold">
                          {result.checks.find(c => c.id === "field_verification")?.detail || t("field_verif_desc")}
                        </p>
                      </div>
                    </div>

                    <span className={`px-3 py-1 rounded-md text-xs font-extrabold border shrink-0 shadow-2xs ${
                      result.checks.find(c => c.id === "field_verification")?.passed === true
                        ? "bg-gradient-to-r from-[#ECE6DA] via-[#f2ece0] to-[#e4ded0] text-[#4E6158] border-[#CACEB5]"
                        : result.checks.find(c => c.id === "field_verification")?.passed === false
                        ? "bg-amber-50 text-amber-700 border-amber-200"
                        : "bg-slate-100 text-slate-600 border-slate-200"
                    }`}>
                      {result.checks.find(c => c.id === "field_verification")?.passed === true ? "Passed" : result.checks.find(c => c.id === "field_verification")?.passed === false ? "Mismatch" : "Skipped"}
                    </span>
                  </div>
                  ...
                  {uploadedImage && (
                    <div className="bg-gradient-to-br from-[#f7f4ee] to-[#f0ebdf] border border-[#CACEB5]/80 rounded-xl p-4 flex flex-col justify-center items-center text-center shadow-inner">
                      <img
                        ...
                        className="max-h-48 object-contain rounded-lg shadow-xs border border-[#CACEB5] mb-2"
                      />
                    </div>
                  )}
                </div>

                {/* ROW 3: Check if Tampered (ELA) */}
                <div className="border border-[#CACEB5] rounded-2xl p-5 bg-gradient-to-b from-white via-[#faf8f4] to-[#f4efe6]/50 space-y-4 hover:border-[#4E6158] shadow-2xs transition-all duration-200">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#ECE6DA] via-[#f0eae0] to-[#e4dccb] text-[#4E6158] flex items-center justify-center shrink-0 border border-[#CACEB5] shadow-xs">
                      ...
                      <div>
                        <h4 className="text-sm font-black text-[#1F2532]">Error Level Analysis (ELA Tampering)</h4>
                        <p className="text-xs text-[#2F3543] mt-0.5 leading-relaxed font-semibold">
                          {result.checks.find(c => c.id === "tampering")?.detail || t("tampering_check_desc")}
                        </p>
                      </div>
                    </div>

                    <span className={`px-3 py-1 rounded-md text-xs font-extrabold border shrink-0 shadow-2xs ${
                      result.checks.find(c => c.id === "tampering")?.passed
                        ? "bg-gradient-to-r from-[#ECE6DA] via-[#f2ece0] to-[#e4ded0] text-[#4E6158] border-[#CACEB5]"
                        : "bg-red-50 text-red-700 border-red-200"
                    }`}>
                      {result.checks.find(c => c.id === "tampering")?.passed ? "Clean" : "Tampered"}
                    </span>
                  </div>

                  {/* Heatmap Viewer if available */}
                  {result.checks.find(c => c.id === "tampering")?.heatmap_path && (
                    <div className="bg-gradient-to-br from-[#f7f4ee] to-[#f0ebdf] border border-[#CACEB5]/80 rounded-xl p-4 flex flex-col items-center shadow-inner">
                      <p className="text-[11px] font-black text-[#2F3543] mb-2 uppercase tracking-wide">
                        Compression Anomaly Heatmap (Bounding Box Detection)
                      </p>
                      <img
                        src={result.checks.find(c => c.id === "tampering")?.heatmap_path!}
                        alt="ELA Compression Heatmap"
                        className="max-h-56 object-contain rounded-lg border border-[#CACEB5] shadow-xs"
                      />
                    </div>
                  )}
                </div>
